    ...
    *,
    ...
    if str(algo) == "fedevo":
        ckpt["fedevo"] = {
            ...
            "theta_fedavg": runner.theta_fedavg,
...
def main() -> None:
    ...
    parser.add_argument("--weight_decay", type=float, default=5e-4, help="Weight decay (paper default: 5e-4)")
    ...
    # Base fieldnames + per-slot usage columns (slot labels are fixed for m=10)
    # Slots: 0=stab 1=used 2=topk 3-6=interp0-3 7=orth0 8-9=seed0-1
    _m = int(args.m)
    _n_interp = int(args.num_interp)
    _n_orth = int(args.num_orth)
    _slot_labels = (
        ["stab", "used", "topk"]
        + [f"interp{i}" for i in range(_n_interp)]
        + [f"orth{i}" for i in range(_n_orth)]
    )
    while len(_slot_labels) < _m:
        _slot_labels.append(f"seed{len(_slot_labels) - 3 - _n_interp - _n_orth}")
    _slot_labels = _slot_labels[:_m]

    _usage_cols = [f"usage_{lbl}" for lbl in _slot_labels]
    _fieldnames = (
        ["round", "test_accuracy", "train_loss", "uplink_bytes", "learning_rate"]
        + _usage_cols
        + ["entropy", "tau", "top1_slot", "top1_label", "orth_active", "mut_active"]
    )
    ...
    with open(out_csv_path, csv_mode, newline="") as f:
        ...
        for r in range(int(r_start), int(args.rounds)):
            ...
            # Build row dict with tracking info for fedevo runs
            row = {
                "round": r,
            ...
            if args.algo == "fedevo":
                usage = getattr(runner, "last_usage_counts", [0] * _m)
                entropy = getattr(runner, "last_entropy", 0.0)
                tau_val = getattr(runner, "last_tau", 0.0)
                orth_active = getattr(runner, "last_orth_active", False)
                mut_active = getattr(runner, "last_mut_active", False)
                slot_labels_now = getattr(runner, "last_slot_labels", _slot_labels)

                top1_idx = int(max(range(len(usage)), key=lambda i: usage[i]))
                top1_lbl = slot_labels_now[top1_idx] if top1_idx < len(slot_labels_now) else str(top1_idx)

                for col, lbl in zip(_usage_cols, _slot_labels):
                    slot_idx = _slot_labels.index(lbl) if lbl in _slot_labels else -1
                    row[col] = usage[slot_idx] if 0 <= slot_idx < len(usage) else 0

                row["entropy"] = f"{entropy:.4f}"
                row["tau"] = f"{tau_val:.4f}"
                row["top1_slot"] = top1_idx
                row["top1_label"] = top1_lbl
                row["orth_active"] = int(orth_active)
                row["mut_active"] = int(mut_active)
            else:
