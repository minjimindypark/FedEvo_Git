...
class FedEvoRunner:
    ...
    def run_round(
        ...
        # Track model selection for ablation analysis.
        # Population slot layout (fixed for m=10, num_interp=4, num_orth=1):
        #   0: stab  1: used  2: topk  3-6: interp  7: orth  8-9: seed_fill
        n_fixed = 3  # stab, used, topk
        n_interp = int(self.ga.num_interp)
        n_orth = int(self.ga.num_orth)
        slot_labels: List[str] = (
            ["stab", "used", "topk"]
            + [f"interp{i}" for i in range(n_interp)]
            + [f"orth{i}" for i in range(n_orth)]
        )
        while len(slot_labels) < int(self.ga.m):
            slot_labels.append(f"seed{len(slot_labels) - n_fixed - n_interp - n_orth}")

        self.last_slot_labels = slot_labels[: int(self.ga.m)]
        self.last_orth_active = bool(enable_orth_now)
        self.last_mut_active = bool(enable_mut_now and H < tau)
