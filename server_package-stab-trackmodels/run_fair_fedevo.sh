...
# Track version: logs per-slot usage_counts, entropy, top1_label, orth/mut flags each round
...
mkdir -p "$OUT_DIR"
OUT_CSV="$OUT_DIR/fedevo_stab_track_cifar10_alpha${ALPHA_STR}_seed${SEED}.csv"
...
echo "[RUN] FedEvo(stab, track) | alpha=$ALPHA | seed=$SEED"
